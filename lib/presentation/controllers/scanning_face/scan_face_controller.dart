...
class ScanFaceController extends GetxController {
    ...
    try {
      ...
      try {
        deviceOrientationDegrees =
            _orientationDegrees[cameraController?.value.deviceOrientation ?? DeviceOrientation.portraitUp];
      } catch (_) {
      ...
      if (kDebugMode) {
        ...
        debugPrint('Sensor orientation: ${cameraController?.description.sensorOrientation}');
        try {
          for (var i = 0; i < image.planes.length; i++) {
            final p = image.planes[i];
            debugPrint('Plane $i bytesPerRow=${p.bytesPerRow} bytesPerPixel=${p.bytesPerPixel} len=${p.bytes.length}');
          }
        } catch (_) {}
      }
  ...
  void _evaluateFacePosition(
    ...
    final px = (mappedX * previewWidgetSize.width).clamp(0.0, previewWidgetSize.width);
    final py = (mappedY * previewWidgetSize.height).clamp(0.0, previewWidgetSize.height);
  ...
  void _updateStateOutsideCircle() {
    ...
    message.value = 'Make sure your face is in the center of the circle, or your face is not in the circle.';
  }
